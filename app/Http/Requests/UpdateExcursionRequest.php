...
class UpdateExcursionRequest extends FormRequest
{
    ...
    {
        // Get the excursion ID from the route - try 'excursion' first, then fallback to route parameters
        $excursionId = $this->route('excursion') ?? $this->route('id') ?? null;
        
        return [
            'title' => 'sometimes|string|max:255|unique:excursions,title,' . ($excursionId ?? ''),
            // Allow empty/null descriptions on update; minimum length only when provided
            ...
            'children_price' => 'sometimes|nullable|numeric|min:0|max:99999999.99',
